...
public class ICombatObject_Area : ICombatObject, AreaController
{
    ...
    public ICombatObject_Area(GameObject path, Vector3 pos)
    {
        SetupAreaObj(path, pos);
    }

    protected virtual void SetupAreaObj(GameObject path, Vector3 pos)
    {
        GameObject obj = UnityEngine.Object.Instantiate(path, pos, Quaternion.identity);
        obj.SetActive(false);
        obj.transform.parent = Combat.instanceTransfrom;
        prefab = obj.GetComponent<Area>();
        prefab.InitArea(this,this);
    }
