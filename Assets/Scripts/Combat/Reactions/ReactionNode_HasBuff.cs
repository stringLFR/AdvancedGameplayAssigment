...
public class ReactionNode_HasBuff : ActionNodeBase
{
    public void SetHasBuffPrefabPath(String PrefabPath)
    {
        HasBuffPrefabPath = PrefabPath;

        effect.SetAssetPath(HasBuffPrefabPath);
    }

    public void SetTargetBuff(BuffsEnum target)
    {
        targetBuff = target;
    }

    private string HasBuffPrefabPath;
    ...
    public override float GetInputScore(CombatListener input)
    {
        ...
        if (target.GetTargetBuffValue(targetBuff) > 0) return caster.MyHP <= 0 ? 0f : minimumInputActivationScore + UnityEngine.Random.Range(minimumInputActivationScore / 2, minimumInputActivationScore + minimumInputActivationScore / 2);
