...
public class ReactionNode_HasStatus : ActionNodeBase
{
    ...
    public override float GetInputScore(CombatListener input)
    {
        ...
        if (target.GetTargetStatus(targetStatus) == true) return caster.MyHP <= 0 ? 0f : minimumInputActivationScore + UnityEngine.Random.Range(minimumInputActivationScore / 2, minimumInputActivationScore + minimumInputActivationScore / 2);
